...
function LazyImg({ src, alt, style, onClick }) {
  ...
  return (
    <div
      ref={ref}
      style={{ ...style, background: "#F0EDE8", position: "relative", overflow: "hidden" }}
      onClick={onClick}
    >
      {vis && (
        <img
            ...
            {filtered.map((p) => {
              ...
              return (
                ...
                >
                  ...
                  <div style={{ height: 110, background: "#F5F2ED" }}>
                    {hasImg ? (
                      <LazyImg src={p.media[0].url} alt={nm} style={{ width: "100%", height: 110 }} />
                    ) : (
                      <div style={{ height: 110, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 48 }}>
                        {p.emoji || p.image || "📦"}
                      </div>
                    )}
                  </div>
                  ...
                  <div key={item.id} style={{ background: C.card, borderRadius: 12, padding: 10, marginBottom: 8, display: "flex", gap: 10, alignItems: "center", border: `1px solid ${C.bdr}` }}>
                    {hasImg ? (
                      <div style={{ width: 50, height: 50, borderRadius: 8, overflow: "hidden", flexShrink: 0 }}>
                        <img src={item.media[0].url} alt={nm} loading="lazy" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                      </div>
                    ) : (
