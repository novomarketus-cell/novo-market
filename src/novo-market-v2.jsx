...
const CATEGORIES = {
  en: ["All","Baby Food","Snacks","Beverages","Seasoning","Noodles","Rice","Sauce"],
  ko: ["전체","이유식","간식","음료","조미료","면류","밥/죽","소스"],
};

const PRODUCTS = [
  { id:1, name_en:"Organic Baby Rice Porridge", name_ko:"유기농 쌀미음", cat:1, price:8.99, stock:25, emoji:"🍚",
    desc_en:"Stage 1 (4+ months). 100% organic Korean rice.", desc_ko:"1단계 (4개월+). 100% 유기농 국내산 쌀.",
    promo:null, tiered:[{qty:3,price:7.49},{qty:5,price:6.99}],
    tags:[{label:"유기농",color:"#FFF8E1",textColor:"#F57F17"},{label:"무첨가",color:"#E3F2FD",textColor:"#1565C0"}],
    media:[
      {type:"image",url:"https://images.unsplash.com/photo-1586201375761-83865001e31c?w=400&q=75"},
      {type:"image",url:"https://images.unsplash.com/photo-1505253716362-afaea1d3d1af?w=400&q=75"},
      {type:"image",url:"https://images.unsplash.com/photo-1536304929831-ee1ca9d44906?w=400&q=75"},
    ]},
  { id:2, name_en:"Sweet Potato & Carrot Puree", name_ko:"고구마당근 퓨레", cat:1, price:9.49, stock:18, emoji:"🍠",
    desc_en:"Stage 2 (6+ months). No additives.", desc_ko:"2단계 (6개월+). 무첨가.",
    promo:null, tiered:null,
    media:[{type:"image",url:"https://images.unsplash.com/photo-1590165482129-1b8b27698780?w=400&q=75"}]},
  { id:3, name_en:"Beef & Veggie Baby Food", name_ko:"소고기야채 이유식", cat:1, price:11.99, stock:12, emoji:"🥩",
    desc_en:"Stage 3 (9+ months). Korean beef with vegetables.", desc_ko:"3단계 (9개월+). 국내산 한우와 제철 채소.",
    promo:{type:"sale",value:10}, tiered:null,
    media:[
      {type:"image",url:"https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=400&q=75"},
      {type:"video",url:"https://www.w3schools.com/html/mov_bbb.mp4",poster:"https://images.unsplash.com/photo-1504674900247-0877df9cc836?w=400&q=75"},
    ]},
  { id:4, name_en:"Rice Puff Snack", name_ko:"쌀과자 오리지널", cat:2, price:4.99, stock:40, emoji:"🍘",
    desc_en:"Melt-in-mouth rice puffs for babies 6+ months.", desc_ko:"6개월 이상 아기를 위한 쌀과자.",
    promo:null, tiered:null,
    media:[{type:"image",url:"https://images.unsplash.com/photo-1558618666-fcd25c85f82e?w=400&q=75"}]},
  { id:5, name_en:"Seaweed Snack for Kids", name_ko:"김스낵", cat:2, price:5.49, stock:35, emoji:"🌿",
    desc_en:"Lightly seasoned Korean seaweed.", desc_ko:"살짝 간을 한 한국산 김.",
    promo:null, tiered:[{qty:4,price:4.49},{qty:8,price:3.99}],
    tags:[{label:"저염",color:"#E8F5E9",textColor:"#2E7D32"}],
    media:[{type:"image",url:"https://images.unsplash.com/photo-1574484284002-952d92456975?w=400&q=75"}]},
  { id:6, name_en:"Fruit Jelly Pouch (10pk)", name_ko:"과일젤리 파우치", cat:2, price:12.99, stock:0, emoji:"🍇",
    desc_en:"Assorted fruit jelly pouches.", desc_ko:"모듬 과일 젤리 파우치.",
    promo:{type:"sale",value:15}, tiered:null,
    media:[{type:"image",url:"https://images.unsplash.com/photo-1601004890684-d8cbf643f5f2?w=400&q=75"}]},
  { id:7, name_en:"Korean Barley Tea (30 bags)", name_ko:"보리차 (30티백)", cat:3, price:7.99, stock:50, emoji:"🍵",
    desc_en:"Caffeine-free roasted barley tea.", desc_ko:"카페인 없는 볶은 보리차.",
    promo:null, tiered:[{qty:3,price:6.99},{qty:5,price:5.99}],
    media:[{type:"image",url:"https://images.unsplash.com/photo-1544787219-7f47ccb76574?w=400&q=75"}]},
  { id:8, name_en:"Yuja Honey Tea", name_ko:"유자차", cat:3, price:14.99, stock:8, emoji:"🍋",
    desc_en:"Traditional Korean citron tea with honey.", desc_ko:"전통 유자차. 꿀 함유.",
    promo:null, tiered:null,
    media:[{type:"image",url:"https://images.unsplash.com/photo-1556679343-c7306c1976bc?w=400&q=75"}]},
  { id:9, name_en:"Anchovy Powder for Baby", name_ko:"아기 멸치가루", cat:4, price:9.99, stock:22, emoji:"🐟",
    desc_en:"Finely ground anchovy powder.", desc_ko:"곱게 갈은 멸치가루.",
    promo:null, tiered:null,
    tags:[{label:"무염",color:"#E8F5E9",textColor:"#2E7D32"},{label:"유통기한 짧음",color:"#FDEDED",textColor:"#C0392B"}],
    media:[{type:"image",url:"https://images.unsplash.com/photo-1615141982883-c7ad0e69fd62?w=400&q=75"}]},
  { id:10, name_en:"Sesame Oil (Baby Grade)", name_ko:"참기름 (이유식용)", cat:4, price:11.49, stock:15, emoji:"🫗",
    desc_en:"Cold-pressed sesame oil.", desc_ko:"냉압착 참기름.",
    promo:{type:"sale",value:20}, tiered:null,
    media:[{type:"image",url:"https://images.unsplash.com/photo-1474979266404-7eaacbcd87c5?w=400&q=75"}]},
  { id:11, name_en:"Baby Udon Noodles", name_ko:"유아 우동면", cat:5, price:6.49, stock:30, emoji:"🍜",
    desc_en:"Soft udon noodles. No salt.", desc_ko:"유아용 부드러운 우동면. 무염.",
    promo:null, tiered:null,
    media:[{type:"image",url:"https://images.unsplash.com/photo-1618841557871-b4664fbf0cb3?w=400&q=75"}]},
  { id:12, name_en:"Veggie Somyeon", name_ko:"야채 소면", cat:5, price:5.99, stock:28, emoji:"🍝",
    desc_en:"Thin noodles with spinach, carrot, pumpkin.", desc_ko:"시금치, 당근, 단호박 소면.",
    promo:null, tiered:null, media:[]},
  { id:13, name_en:"Instant Baby Rice Bowl", name_ko:"즉석 아기 덮밥", cat:6, price:7.49, stock:20, emoji:"🍱",
    desc_en:"Ready-to-eat with mild curry.", desc_ko:"순한 카레소스 즉석 덮밥.",
    promo:null, tiered:null, media:[]},
  { id:14, name_en:"Seaweed Soup Rice Pack", name_ko:"미역국밥", cat:6, price:8.99, stock:16, emoji:"🥣",
    desc_en:"Just heat and serve.", desc_ko:"데우기만 하면 완성.",
    promo:null, tiered:null, media:[]},
  { id:15, name_en:"Mild Soy Sauce for Kids", name_ko:"어린이 간장", cat:7, price:6.99, stock:33, emoji:"🫙",
    desc_en:"Reduced sodium for children.", desc_ko:"어린이를 위한 저염 간장.",
    promo:null, tiered:null, media:[]},
  { id:16, name_en:"Baby Tomato Ketchup", name_ko:"유아 토마토 케찹", cat:7, price:5.49, stock:27, emoji:"🍅",
    desc_en:"No sugar, no colors. Real tomatoes.", desc_ko:"무설탕, 무색소.",
    promo:null, tiered:null, media:[]},
];

const FREE_SHIP = 150;
const SHIP_COST = 9.99;
const COUPONS = { WELCOME10:{type:"percent",value:10}, NOVO5:{type:"flat",value:5} };
...
const basePrice = p => p.promo ? +(p.price*(1-p.promo.value/100)).toFixed(2) : p.price;
...
// Phone format helper: (xxx)xxx-xxxx
...
// English-only filter: strip non-ASCII (Korean etc.)
const englishOnly = (val) => val.replace(/[^\x20-\x7E]/g, "");

const T = {
  en:{brand:"NOVO MARKET",tag:"Premium Korean Baby & Kids Food",search:"Search products...",cart:"Cart",add:"Add to Cart",sold:"Sold Out",off:"OFF",sub:"Subtotal",ship:"Shipping",free:"FREE",freeNote:`Free shipping on orders $${FREE_SHIP}+`,total:"Total",checkout:"Place Order",empty:"Your cart is empty",info:"Order Information",name:"Full Name",phone:"Phone Number",addr:"Street Address",city:"City",state:"State",zip:"ZIP",vname:"Venmo / Zelle Name",save:"Save my info",submit:"Submit Order",done:"Order Submitted!",orderNum:"Order Number",amt:"Amount to Pay",payMsg:"Send payment via Venmo or Zelle with your order number in the memo, then tap 'I Paid' below.",paid:"I Paid ✓",paidOk:"Payment notification sent! We'll confirm and ship soon.",track:"Track",trackNum:"Tracking #",trackBtn:"Track Package →",history:"Orders",enterPhone:"Enter phone to view orders",none:"No orders found",promo:"Promo code",apply:"Apply",remove:"Remove",back:"← Back",moreFree:"more for free shipping!",ok:"Applied!",bad:"Invalid code",added:"Added!",shop:"Shop",items:"items",link:"Link to existing order",linkNum:"Existing Order #",linkSubmit:"Link & Submit",memo:"Write order # in Venmo/Zelle memo",shopNow:"Shop Now",bulk:"Buy more, save more!",bulkTag:"Bulk discount",ea:"ea",cur:"Current price",
    ...
    englishOnly:"English only please",
  },
  ko:{brand:"노보마켓",tag:"프리미엄 한국 유아·아동 식품",search:"제품 검색...",cart:"장바구니",add:"담기",sold:"품절",off:"할인",sub:"소계",ship:"배송비",free:"무료",freeNote:`$${FREE_SHIP} 이상 무료배송`,total:"합계",checkout:"주문하기",empty:"장바구니가 비어있습니다",info:"주문 정보",name:"이름",phone:"전화번호",addr:"주소",city:"도시",state:"주(State)",zip:"우편번호",vname:"Venmo / Zelle 표시 이름",save:"다음 주문을 위해 정보 저장",submit:"주문 제출",done:"주문 접수 완료!",orderNum:"주문번호",amt:"결제 금액",payMsg:"Venmo 또는 Zelle로 결제 시 메모란에 주문번호를 적어주세요. 결제 후 아래 버튼을 눌러주세요.",paid:"결제 완료 ✓",paidOk:"결제 알림이 전송되었습니다! 확인 후 발송해 드리겠습니다.",track:"배송조회",trackNum:"운송장번호",trackBtn:"배송 추적 →",history:"주문내역",enterPhone:"전화번호 입력 시 주문내역 조회",none:"주문 내역 없음",promo:"쿠폰 코드",apply:"적용",remove:"삭제",back:"← 뒤로",moreFree:"추가 시 무료배송!",ok:"쿠폰 적용!",bad:"유효하지 않은 쿠폰",added:"담았습니다!",shop:"홈",items:"개",link:"기존 주문에 연결",linkNum:"기존 주문번호",linkSubmit:"연결 주문",memo:"Venmo/Zelle 메모에 주문번호를 꼭 적어주세요",shopNow:"쇼핑하기",bulk:"많이 사면 더 저렴해요!",bulkTag:"묶음할인",ea:"개",cur:"현재 적용가",
...
function Gallery({media,emoji,name}){
  ...
  // Auto-height: load image natural size and calculate aspect ratio
  const handleImgLoad = (e) => {
    const img = e.target;
    if (img.naturalWidth && img.naturalHeight) {
      const containerWidth = imgRef.current?.clientWidth || 400;
      const ratio = img.naturalHeight / img.naturalWidth;
      const calcH = Math.min(Math.max(containerWidth * ratio, 200), 500);
      setImgH(calcH);
    }
  };
...
export default function NovoMarket(){
  ...
  // Accurate cart price calculation: each item uses its own tiered price based on qty
  const subtotal = useMemo(() => {
    return cart.reduce((sum, item) => {
      const unitPrice = tieredPrice(item, item.qty);
      return sum + (Math.round(unitPrice * 100) * item.qty) / 100;
    }, 0);
  }, [cart]);
  ...
  const filtered=useMemo(()=>PRODUCTS.filter(p=>{
    if(cat>0&&p.cat!==cat)return false;
    const n=lang==="en"?p.name_en:p.name_ko;
    if(search&&!n.toLowerCase().includes(search.toLowerCase()))return false;
    return true;
  }),[cat,search,lang]);

  // English-only input handler for customer info fields
  const handleInfoChange = (key, val) => {
    if (key === "phone") {
      setInfo(prev => ({ ...prev, phone: formatPhone(val) }));
    } else if (key === "state") {
      // State: uppercase, max 2 chars
      setInfo(prev => ({ ...prev, state: englishOnly(val).toUpperCase().slice(0, 2) }));
    } else if (key === "zip") {
      // ZIP: digits only, max 5
      setInfo(prev => ({ ...prev, zip: val.replace(/\D/g, "").slice(0, 5) }));
    } else {
      // name, addr, city, vname: English only
      setInfo(prev => ({ ...prev, [key]: englishOnly(val) }));
    }
  };

  // Check if input has Korean characters
  const hasKorean = (val) => /[\u3131-\u3163\uac00-\ud7a3]/g.test(val);

  const C={bg:"#F7FAFB",card:"#FFF",pri:"#6AADCC",priL:"#EAF5FA",acc:"#E8879A",accBg:"#FFF0F3",
  ...
  // Submit handler with uppercase conversion
  const handleSubmitOrder = () => {
    const orderData = {
      ...
      items: cart.map(item => ({
        ...item,
        name: lang === "en" ? item.name_en : item.name_ko,
        price: tieredPrice(item, item.qty),
      })),
      linkedTo: linkMode ? linkNum : null,
      customer: {
        name: info.name.toUpperCase(),
        phone: info.phone,
        address: info.addr.toUpperCase(),
        city: info.city.toUpperCase(),
  ...
  return <div style={{fontFamily:"'Nunito',sans-serif",background:C.bg,minHeight:"100vh",maxWidth:480,margin:"0 auto",color:C.txt,position:"relative",paddingBottom:70}}>
    ...
    {showCancelPolicy&&<div onClick={()=>setShowCancelPolicy(false)} style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#FFF",borderRadius:16,maxWidth:400,width:"100%",padding:24}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:16}}>
          <h3 style={{margin:0,fontSize:16,fontWeight:800}}>📋 {t.cancelPolicy}</h3>
          <button onClick={()=>setShowCancelPolicy(false)} style={{background:"none",border:"none",fontSize:18,cursor:"pointer",color:C.light}}>✕</button>
        </div>
    ...
    <div style={{background:"#FFF",padding:"10px 16px 8px",position:"sticky",top:0,zIndex:100,boxShadow:"0 1px 6px rgba(106,173,204,.1)",borderBottom:`1px solid ${C.bdr}`}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:4}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          <button onClick={()=>setShowCancelPolicy(true)} style={{...B,background:"none",color:C.light,padding:"4px 8px",fontSize:10,border:`1px solid ${C.bdr}`}}>📋 {t.cancelPolicy}</button>
      ...
      <div onClick={()=>{setPage("shop");setSelProd(null);}} style={{cursor:"pointer",textAlign:"center"}}>
        ...
        <div style={{fontFamily:"'Playfair Display',serif",fontSize:15,fontWeight:700,color:C.txt,letterSpacing:1}}>NOVO MARKET</div>
    ...
    {selProd&&page==="shop"&&(()=>{
      ...
      if(!p)return null;
      const nm=lang==="en"?p.name_en:p.name_ko;
      const dc=lang==="en"?p.desc_en:p.desc_ko;
      ...
      return <div style={{padding:16,paddingBottom:100}}>
        ...
        <Gallery media={p.media} emoji={p.emoji} name={nm}/>
        <div style={{marginTop:16}}>
          {p.promo&&<span style={{background:C.danger,color:"#FFF",fontSize:11,fontWeight:700,padding:"3px 10px",borderRadius:5,marginBottom:8,display:"inline-block"}}>{p.promo.value}% {t.off}</span>}
          <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:22,margin:"8px 0 6px",lineHeight:1.3}}>{nm}</h2>
          {p.tags&&p.tags.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:5,marginBottom:8}}>
            {p.tags.map((tag,i)=><span key={i} style={{fontSize:11,fontWeight:600,padding:"3px 10px",borderRadius:20,background:tag.color||"#EEE",color:tag.textColor||"#555"}}>{tag.label}</span>)}
          </div>}
          ...
          {p.tiered&&p.tiered.length>0&&<div style={{background:C.priL,borderRadius:10,padding:"10px 14px",marginBottom:14}}>
            ...
            <div style={{display:"flex",gap:6}}>
              <div style={{flex:1,textAlign:"center",padding:"8px 4px",borderRadius:8,background:(!ic||ic.qty<p.tiered[0].qty)?C.pri:"#FFF",color:(!ic||ic.qty<p.tiered[0].qty)?"#FFF":C.txt,border:`1px solid ${C.pri}`}}>
                <div style={{fontSize:10,opacity:.8}}>1{t.ea}</div>
              ...
              {p.tiered.map((tier,i)=>{
                ...
                return <div key={i} style={{flex:1,textAlign:"center",padding:"8px 4px",borderRadius:8,background:active?C.pri:"#FFF",color:active?"#FFF":C.txt,border:`1px solid ${C.pri}`,position:"relative"}}>
                  ...
                  <div style={{fontSize:10,opacity:.8}}>{tier.qty}+</div>
    ...
    {/* SHOP */}
    {page==="shop"&&!selProd&&<div style={{padding:"12px 16px 20px"}}>
      <div style={{background:C.wh,borderRadius:10,border:`1px solid ${C.bdr}`,padding:"9px 12px",display:"flex",alignItems:"center",gap:8,marginBottom:10}}>
        <span>🔍</span>
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={t.search} style={{border:"none",outline:"none",flex:1,fontSize:13,fontFamily:"'Nunito',sans-serif",background:"transparent"}}/>
      </div>
      <div style={{display:"flex",gap:5,overflowX:"auto",paddingBottom:10}}>
        {CATEGORIES[lang].map((c,i)=><button key={i} onClick={()=>setCat(i)} style={{...B,padding:"5px 12px",fontSize:11,whiteSpace:"nowrap",background:cat===i?C.pri:C.wh,color:cat===i?"#FFF":C.light,border:`1px solid ${cat===i?C.pri:C.bdr}`}}>{c}</button>)}
      </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        {filtered.map(p=>{
          ...
          return <div key={p.id} onClick={()=>setSelProd(p.id)} style={{background:C.card,borderRadius:12,overflow:"hidden",border:`1px solid ${C.bdr}`,position:"relative",cursor:"pointer",opacity:so?.6:1,boxShadow:"0 1px 4px rgba(0,0,0,.04)"}}>
            {p.promo&&!so&&<div style={{position:"absolute",top:6,left:6,background:C.danger,color:"#FFF",fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:4,zIndex:2}}>{p.promo.value}% {t.off}</div>}
            ...
            {/* Card image: height 150, objectFit contain */}
            <div style={{height:150,background:"#F5F2ED"}}>
              {hasImg?<LazyImg src={p.media[0].url} alt={nm} style={{width:"100%",height:150}}/>
              :<div style={{height:150,display:"flex",alignItems:"center",justifyContent:"center",fontSize:48}}>{p.emoji}</div>}
            </div>
            <div style={{padding:"8px 10px 10px"}}>
              <div style={{fontSize:11,fontWeight:700,lineHeight:1.3,marginBottom:5,minHeight:28}}>{nm}</div>
              <div style={{display:"flex",alignItems:"center",gap:5,marginBottom:4}}>
                {p.promo?<><span style={{fontSize:14,fontWeight:800,color:C.danger}}>{fmt(pr)}</span><span style={{fontSize:10,color:C.light,textDecoration:"line-through"}}>{fmt(p.price)}</span></>
                :<span style={{fontSize:14,fontWeight:800,color:C.pri}}>{fmt(pr)}</span>}
              </div>
              {p.tiered&&p.tiered.length>0&&<div style={{fontSize:9,color:C.pri,fontWeight:600,marginBottom:5,background:C.priL,padding:"2px 6px",borderRadius:4,display:"inline-block"}}>📦 {p.tiered[0].qty}+ {fmt(p.tiered[0].price)}</div>}
              {p.tags&&p.tags.length>0&&<div style={{display:"flex",flexWrap:"wrap",gap:3,marginBottom:5}}>
              ...
              <button onClick={e=>{e.stopPropagation();if(!so)addToCart(p);}} disabled={so} style={{...B,width:"100%",padding:7,fontSize:11,background:so?"#CCC":C.pri,color:"#FFF"}}>{so?t.sold:t.add}</button>
            </div>
          </div>;
        })}
      </div>
    ...
    {/* CART */}
    {page==="cart"&&step===0&&<div style={{padding:16}}>
      ...
      {cart.length===0?<div style={{textAlign:"center",padding:"40px 0",color:C.light}}>
        <div style={{fontSize:40,marginBottom:8}}>🛒</div><div>{t.empty}</div>
        <button onClick={()=>setPage("shop")} style={{...B,marginTop:16,background:C.pri,color:"#FFF",padding:"10px 24px",fontSize:13}}>{t.shopNow}</button>
      </div>:<>
        {cart.map(item=>{
          const bp2=basePrice(item);const tp2=tieredPrice(item,item.qty);const isTiered=tp2<bp2;
          const nm=lang==="en"?item.name_en:item.name_ko;
          const hasImg=item.media&&item.media[0]?.type==="image";
          const lineTotal = Math.round(tp2 * item.qty * 100) / 100;
          return <div key={item.id} style={{background:C.card,borderRadius:12,padding:10,marginBottom:8,display:"flex",gap:10,alignItems:"center",border:`1px solid ${C.bdr}`}}>
            {hasImg?<div style={{width:50,height:50,borderRadius:8,overflow:"hidden",flexShrink:0}}><img src={item.media[0].url} alt={nm} loading="lazy" style={{width:"100%",height:"100%",objectFit:"contain"}}/></div>
            :<div style={{fontSize:30,width:50,textAlign:"center",flexShrink:0}}>{item.emoji}</div>}
            <div style={{flex:1,minWidth:0}}>
              ...
              <div style={{display:"flex",alignItems:"center",gap:4}}>
                ...
                <span style={{fontSize:11,color:C.mid,marginLeft:4}}>= {fmt(lineTotal)}</span>
              </div>
        ...
        <div style={{background:C.card,borderRadius:10,padding:10,margin:"10px 0",border:`1px solid ${C.bdr}`}}>
          <div style={{display:"flex",gap:6}}>
            ...
            <button onClick={()=>{const c=COUPONS[couponIn.trim().toUpperCase()];if(c){setCoupon(c);setToast(t.ok);}else setToast(t.bad);}} style={{...B,background:C.pri,color:"#FFF",padding:"8px 14px",fontSize:12}}>{t.apply}</button>
          </div>
          {coupon&&<div style={{marginTop:6,fontSize:11,color:C.pri,fontWeight:600,display:"flex",justifyContent:"space-between"}}>
        ...
        <div style={{background:C.card,borderRadius:12,padding:14,border:`1px solid ${C.bdr}`}}>
          ...
          {linkMode&&<input value={linkNum} onChange={e=>setLinkNum(e.target.value)} placeholder={t.linkNum} style={{width:"100%",border:`1px solid ${C.bdr}`,borderRadius:8,padding:"8px 10px",fontSize:13,fontFamily:"'Nunito',sans-serif",boxSizing:"border-box",marginTop:6}}/>}
    ...
    {/* CHECKOUT */}
    {page==="cart"&&step===1&&<div style={{padding:16,paddingBottom:100}}>
      ...
      <div style={{background:C.card,borderRadius:14,padding:14,border:`1px solid ${C.bdr}`,marginBottom:16}}>
        ...
        {cart.map(item=>{const nm=lang==="en"?item.name_en:item.name_ko;const pr=tieredPrice(item,item.qty);
          return <div key={item.id} style={{display:"flex",justifyContent:"space-between",fontSize:13,padding:"3px 0"}}><span>{nm} × {item.qty}</span><span style={{fontWeight:700}}>{fmt(Math.round(pr*item.qty*100)/100)}</span></div>;})}
        ...
        {linkMode&&linkNum&&<div style={{fontSize:12,color:C.acc,fontWeight:600,marginTop:6}}>🔗 Linked to {linkNum}</div>}
      </div>

      {/* English-only notice */}
      <div style={{background:"#FFF8E1",borderRadius:8,padding:"8px 12px",marginBottom:14,fontSize:11,color:"#F57F17",fontWeight:600}}>
        ✍️ {t.englishOnly}
      </div>

      {[{k:"name",l:t.name,ty:"text",ph:"John Doe"},{k:"phone",l:t.phone,ty:"tel",ph:"(123)456-7890"},{k:"vname",l:t.vname,ty:"text",ph:"@venmo-name"},{k:"addr",l:t.addr,ty:"text",ph:"123 Main St, Apt 4B"}].map(f=>
        <div key={f.k} style={{marginBottom:12}}><label style={{fontSize:11,fontWeight:700,color:C.light,marginBottom:4,display:"block",textTransform:"uppercase",letterSpacing:.5}}>{f.l}</label>
        <input value={info[f.k]} onChange={e=>handleInfoChange(f.k, e.target.value)} type={f.ty} placeholder={f.ph||""} style={{width:"100%",border:`1px solid ${C.bdr}`,borderRadius:10,padding:"10px 14px",fontSize:14,fontFamily:"'Nunito',sans-serif",boxSizing:"border-box",background:"#FFF"}}/></div>)}

      {/* Address warning */}
      <div style={{background:"#FFF3E0",borderRadius:8,padding:"8px 12px",marginBottom:14,fontSize:11,color:"#E67E22",fontWeight:600,lineHeight:1.5}}>
        {t.addrWarn}
      </div>

      <div style={{display:"grid",gridTemplateColumns:"2fr 1fr 1fr",gap:10,marginBottom:12}}>
        {[{k:"city",l:t.city,ph:"Los Angeles"},{k:"state",l:t.state,ph:"CA"},{k:"zip",l:t.zip,ph:"90001"}].map(f=>
          <div key={f.k}><label style={{fontSize:11,fontWeight:700,color:C.light,marginBottom:4,display:"block",textTransform:"uppercase",letterSpacing:.5}}>{f.l}</label>
          <input value={info[f.k]} onChange={e=>handleInfoChange(f.k, e.target.value)} placeholder={f.ph||""} style={{width:"100%",border:`1px solid ${C.bdr}`,borderRadius:10,padding:"10px 14px",fontSize:14,fontFamily:"'Nunito',sans-serif",boxSizing:"border-box",background:"#FFF"}}/></div>)}
      </div>
      ...
      {/* Cancellation policy notice in checkout */}
      <div style={{background:"#F5F5F5",borderRadius:8,padding:"10px 12px",marginTop:14,marginBottom:6,fontSize:11,color:C.mid,lineHeight:1.6}}>
        📋 <strong>{t.cancelPolicy}:</strong> {t.cancelText}
      </div>

      {(()=>{
        const phoneDigits = info.phone.replace(/\D/g, "");
        const ok=info.name&&phoneDigits.length===10&&info.addr&&info.city&&info.state&&info.zip;
        return <button onClick={handleSubmitOrder} disabled={!ok}
          style={{...B,width:"100%",padding:14,marginTop:16,fontSize:15,background:ok?C.pri:"#CCC",color:"#FFF",borderRadius:14,opacity:ok?1:.5,pointerEvents:ok?"auto":"none"}}>{linkMode?t.linkSubmit:t.submit}</button>;
      })()}
    </div>}
    ...
    {page==="cart"&&step===2&&paidDone&&<div style={{padding:20,paddingBottom:100,textAlign:"center"}}>
      ...
      <button onClick={()=>{setPage("shop");setStep(0);setOrder(null);setPaidDone(false);setLinkMode(false);setLinkNum("");}} style={{...B,marginTop:20,padding:"14px 40px",fontSize:15,background:C.pri,color:"#FFF",borderRadius:14}}>🛍 {t.shopNow}</button>
    </div>}
    ...
    {/* HISTORY — uses phone as unique ID */}
    {page==="history"&&<div style={{padding:16}}>
      <button onClick={()=>setPage("shop")} style={{...B,background:"none",color:C.pri,padding:"0 0 14px",fontSize:14}}>{t.back}</button>
      <h2 style={{fontFamily:"'Playfair Display',serif",fontSize:20,marginBottom:16}}>📋 {t.history}</h2>
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        <input value={histPhone} onChange={e=>setHistPhone(formatPhone(e.target.value))} placeholder={t.enterPhone + " (xxx)xxx-xxxx"} style={{flex:1,border:`1px solid ${C.bdr}`,borderRadius:8,padding:10,fontSize:13,fontFamily:"'Nunito',sans-serif"}}/>
        <button style={{...B,background:C.pri,color:"#FFF",padding:"10px 14px",fontSize:12}}>🔍</button>
      </div>
      <div style={{fontSize:11,color:C.light,marginBottom:10,textAlign:"center"}}>{lang==="en"?"Your phone number is used to look up your orders":"전화번호로 주문내역을 조회합니다"}</div>
