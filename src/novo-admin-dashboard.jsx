...
// ─────────────────────────────────────────────
// Modal — 컴포넌트 외부에 정의 (리렌더링 시 재생성 방지)
// ─────────────────────────────────────────────
    ...
    >
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
          ...
          <button onClick={onClose} style={{ background: "none", border: "none", fontSize: 20, cursor: "pointer", color: "#7A8599" }}>✕</button>
        </div>
        {children}
      </div>
    </div>
...
// ─────────────────────────────────────────────
// 🛡️ Separate ProductForm component
// Isolated from parent re-renders so typing works normally
// ─────────────────────────────────────────────
const ProductFormModal = React.memo(function ProductFormModal({ initial, categories, editPid, onSave, onClose, saving, c, inp, btn, btnS }) {
  ...
  const ModalWrap = ({ children, title }) => (
  ...
  return (
    <ModalWrap title={editPid ? "제품 수정" : "새 제품 추가"}>
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 12, fontWeight: 600, color: c.tLight, display: "block", marginBottom: 4 }}>제품명 (한국어)</label>
        <input value={pf.nameKo} onChange={(e) => setPf((prev) => ({ ...prev, nameKo: e.target.value }))} type="text" style={inp} />
      </div>
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 12, fontWeight: 600, color: c.tLight, display: "block", marginBottom: 4 }}>Product Name (EN)</label>
        <input value={pf.nameEn} onChange={(e) => setPf((prev) => ({ ...prev, nameEn: e.target.value }))} type="text" style={inp} />
      </div>
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 12, fontWeight: 600, color: c.tLight, display: "block", marginBottom: 4 }}>가격 ($)</label>
        <input value={pf.price} onChange={(e) => setPf((prev) => ({ ...prev, price: Number(e.target.value) }))} type="number" style={inp} />
      </div>
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 12, fontWeight: 600, color: c.tLight, display: "block", marginBottom: 4 }}>재고</label>
        <input value={pf.stock} onChange={(e) => setPf((prev) => ({ ...prev, stock: Number(e.target.value) }))} type="number" style={inp} />
      </div>
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 12, fontWeight: 600, color: c.tLight, display: "block", marginBottom: 4 }}>이모지 (대체용)</label>
        <input value={pf.image} onChange={(e) => setPf((prev) => ({ ...prev, image: e.target.value }))} type="text" style={inp} />
      </div>

      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 12, fontWeight: 600, color: c.tLight, display: "block", marginBottom: 4 }}>📝 제품 설명 (한국어)</label>
        <textarea value={pf.descKo || ""} onChange={(e) => setPf((prev) => ({ ...prev, descKo: e.target.value }))} placeholder="예: 4개월 이상 아기를 위한 유기농 쌀미음" rows={3} style={{ ...inp, resize: "vertical" }} />
      </div>
      <div style={{ marginBottom: 12 }}>
        <label style={{ fontSize: 12, fontWeight: 600, color: c.tLight, display: "block", marginBottom: 4 }}>📝 Description (EN)</label>
        <textarea value={pf.descEn || ""} onChange={(e) => setPf((prev) => ({ ...prev, descEn: e.target.value }))} placeholder="e.g. Organic rice porridge for babies 4+ months" rows={3} style={{ ...inp, resize: "vertical" }} />
      </div>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
          {(pf.tags || []).map((tag, i) => (
            <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 4, background: tag.color || "#EAF5FA", color: tag.textColor || "#2980B9", padding: "4px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600 }}>
              {tag.label}
              <button onClick={() => setPf((prev) => { const nt = [...(prev.tags || [])]; nt.splice(i, 1); return { ...prev, tags: nt }; })} style={{ background: "none", border: "none", cursor: "pointer", color: "inherit", fontSize: 14, padding: 0, marginLeft: 2 }}>✕</button>
            </span>
          ))}
        </div>
        <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginBottom: 6 }}>
          {[
            ...
            <button
              key={preset.label}
              onClick={() => setPf((prev) => {
                if ((prev.tags || []).some((t) => t.label === preset.label)) return prev;
                return { ...prev, tags: [...(prev.tags || []), preset] };
              })}
        ...
        <div style={{ display: "flex", gap: 6 }}>
          ...
          <button
            onClick={() => { const el = document.getElementById("novo-custom-tag"); if (!el || !el.value.trim()) return; setPf((prev) => ({ ...prev, tags: [...(prev.tags || []), { label: el.value.trim(), color: "#F5F5F5", textColor: "#555" }] })); el.value = ""; }}
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        {(pf.media || []).length < 10 && (
          <div style={{ marginBottom: 10 }}>
            ...
            <input id="novo-file-input" type="file" accept="image/*,video/*" multiple capture="environment" style={{ display: "none" }}
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 8 }}>
              ...
              <button onClick={() => setPf((prev) => ({ ...prev, media: [...(prev.media || []), { type: "image", url: "", alt: "" }] }))} style={{ ...btnS, background: "#7F8C8D", flex: 1 }}>🔗 URL</button>
            </div>
          </div>
        )}
        {(pf.media || []).length > 0 && (
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(90px,1fr))", gap: 8, marginTop: 8 }}>
            {(pf.media || []).map((m, i) => (
              <div key={i} style={{ position: "relative", borderRadius: 8, overflow: "hidden", border: `1px solid ${c.border}`, background: "#F0EDE8" }}>
                {m.type === "video" ? (
                  <div style={{ width: "100%", height: 90, display: "flex", alignItems: "center", justifyContent: "center", background: "#1A1A1A" }}><span style={{ fontSize: 24, color: "#FFF" }}>🎬</span></div>
                ) : m.url ? (
                  ...
                  <div style={{ width: "100%", height: 90, display: "flex", alignItems: "center", justifyContent: "center" }}>
                    <input value={m.url} onChange={(e) => setPf((prev) => { const nm = [...(prev.media || [])]; nm[i] = { ...nm[i], url: e.target.value }; return { ...prev, media: nm }; })} placeholder="URL" style={{ width: "80%", border: `1px solid ${c.border}`, borderRadius: 4, padding: "4px 6px", fontSize: 10, textAlign: "center" }} />
                  </div>
                )}
                <button onClick={() => { setPf((prev) => { const nm = [...(prev.media || [])]; nm.splice(i, 1); return { ...prev, media: nm }; }); setPendingFiles((pf2) => pf2.filter((f) => f.index !== i).map((f) => f.index > i ? { ...f, index: f.index - 1 } : f)); }} style={{ position: "absolute", top: 3, right: 3, width: 20, height: 20, borderRadius: "50%", border: "none", background: "rgba(0,0,0,0.6)", color: "#FFF", fontSize: 11, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>✕</button>
      ...
      <div style={{ marginBottom: 12 }}>
        ...
        <select value={pf.category} onChange={(e) => setPf((prev) => ({ ...prev, category: e.target.value }))} style={inp}>
          {Object.entries(categories).map(([k, v]) => (<option key={k} value={k}>{v}</option>))}
        </select>
      </div>
      <div style={{ display: "flex", gap: 12, marginBottom: 12 }}>
        <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, cursor: "pointer" }}>
          <input type="checkbox" checked={pf.sale} onChange={(e) => setPf((prev) => ({ ...prev, sale: e.target.checked }))} />세일
        </label>
        <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, cursor: "pointer" }}>
          <input type="checkbox" checked={pf.active} onChange={(e) => setPf((prev) => ({ ...prev, active: e.target.checked }))} />활성
        </label>
      </div>
      {pf.sale && (
        <div style={{ marginBottom: 12 }}>
          <label style={{ fontSize: 12, fontWeight: 600, color: c.tLight, display: "block", marginBottom: 4 }}>세일 가격</label>
          <input value={pf.salePrice} onChange={(e) => setPf((prev) => ({ ...prev, salePrice: Number(e.target.value) }))} type="number" style={inp} />
        </div>
      )}
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        {(pf.tiered || []).map((tier, i) => (
          <div key={i} style={{ display: "flex", gap: 6, marginBottom: 6, alignItems: "center" }}>
            <div style={{ flex: 1 }}>
              <input value={tier.qty} onChange={(e) => setPf((prev) => { const nt = [...(prev.tiered || [])]; nt[i] = { ...nt[i], qty: Number(e.target.value) }; return { ...prev, tiered: nt }; })} type="number" placeholder="수량" style={{ ...inp, padding: "6px 10px", fontSize: 12 }} />
            </div>
            <span style={{ fontSize: 12, color: c.tLight }}>개 이상 →</span>
            <div style={{ flex: 1 }}>
              <input value={tier.price} onChange={(e) => setPf((prev) => { const nt = [...(prev.tiered || [])]; nt[i] = { ...nt[i], price: Number(e.target.value) }; return { ...prev, tiered: nt }; })} type="number" step="0.01" placeholder="개당 가격" style={{ ...inp, padding: "6px 10px", fontSize: 12 }} />
            </div>
            <span style={{ fontSize: 12, color: c.tLight }}>$</span>
            <button onClick={() => setPf((prev) => { const nt = [...(prev.tiered || [])]; nt.splice(i, 1); return { ...prev, tiered: nt }; })} style={{ ...btnS, padding: "4px 8px", background: c.danger }}>✕</button>
          </div>
        ))}
        <button onClick={() => setPf((prev) => ({ ...prev, tiered: [...(prev.tiered || []), { qty: 3, price: 0 }] }))} style={{ ...btnS, background: "#3498DB", marginTop: 4 }}>+ 수량 구간 추가</button>
      </div>

      <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 16 }}>
        <button onClick={onClose} style={{ ...btn, background: "transparent", color: c.primary, border: `1px solid ${c.primary}` }}>취소</button>
        <button onClick={() => onSave(pf, pendingFiles)} disabled={saving} style={{ ...btn, opacity: saving ? 0.5 : 1 }}>
          {saving ? "저장 중..." : "저장"}
        </button>
      </div>
    </ModalWrap>
...
export default function Admin() {
  ...
  // 🛡️ Refs to prevent real-time updates from stealing focus during editing
  const formOpenRef = useRef(false);
  const trackingTimers = useRef({});
  const stockTimers = useRef({});

  // Keep ref in sync with form state
  useEffect(() => {
    formOpenRef.current = showPF || showPrF || showCatF;
  }, [showPF, showPrF, showCatF]);
  ...
  useEffect(() => {
    ...
    unsubs.push(
      subscribeProducts((prods) => {
        if (!formOpenRef.current) setProducts(prods);
        setLoaded(true);
      })
    );
    unsubs.push(
      subscribeOrders((ords) => {
        if (!formOpenRef.current) setOrders(ords);
      })
    );
    unsubs.push(
      subscribePromos((prs) => {
        if (!formOpenRef.current) setPromos(prs);
      })
    );
    unsubs.push(
      subscribeCategories((cats) => {
        ...
        if (!formOpenRef.current) setCategories(catMap);
      })
    );
  ...
  // ─────────────────────────────────────────────
  const handleSaveProduct = async (formData, formPendingFiles) => {
    if (!formData.nameEn || !formData.nameKo) {
      noti("제품명을 입력하세요");
      return;
    }
    setSaving(true);
    try {
      ...
      for (const { index, file } of formPendingFiles) {
        try {
      ...
      const productData = {
        ...
        media: cleanMedia,
        tiered: (formData.tiered || []).map((t) => ({ qty: Number(t.qty), price: Number(t.price) })),
  ...
  // 🔥 Order Tracking Number Update (debounced — waits 800ms after typing stops)
  const handleTrackingUpdate = (docId, trackingNum) => {
    // Update local state immediately for responsive UI
    setOrders((prev) => prev.map((o) => o._docId === docId ? { ...o, trackingNum } : o));
    // Debounce the Firestore write
    if (trackingTimers.current[docId]) clearTimeout(trackingTimers.current[docId]);
    trackingTimers.current[docId] = setTimeout(async () => {
      try {
  ...
  // 🔥 Stock Update (debounced — local update immediate, Firestore after 500ms)
      ...
      <div style={{ padding: 20, maxWidth: 1100, margin: "0 auto" }}>
        ...
        {loaded && tab === "products" && (
          <div>
            ...
            {/* Product Form Modal — separate component to prevent focus loss */}
            {showPF && (
              <ProductFormModal
                initial={pf}
