    ...
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",zIndex:1000,display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
      <div style={{background:"#FFF",borderRadius:16,maxWidth:520,width:"100%",maxHeight:"85vh",overflow:"auto",padding:24}}>
        ...
        <div style={{marginBottom:14}}>
          ...
          {(pf.variants||[]).map((v,i)=>(<div key={i} style={{display:"flex",gap:6,marginBottom:6,alignItems:"center"}}><div style={{flex:1}}><input value={v.label} onChange={e=>setPf(p=>{const nv=[...(p.variants||[])];nv[i]={...nv[i],label:e.target.value};return{...p,variants:nv};})} type="text" placeholder="옵션명 (예: 30봉)" style={{...INP,padding:"6px 10px",fontSize:12}}/></div><span style={{fontSize:12,color:C.tLight}}>→</span><div style={{flex:1}}><input value={v.price} onChange={e=>setPf(p=>{const nv=[...(p.variants||[])];nv[i]={...nv[i],price:e.target.value};return{...p,variants:nv};})} type="number" step="0.01" placeholder="가격" style={{...INP,padding:"6px 10px",fontSize:12}}/></div><span style={{fontSize:12,color:C.tLight}}>$</span><button onClick={()=>setPf(p=>{const nv=[...(p.variants||[])];nv.splice(i,1);return{...p,variants:nv};})} style={{...BTNS,padding:"4px 8px",background:C.danger}}>✕</button></div>))}
          <button onClick={()=>setPf(p=>({...p,variants:[...(p.variants||[]),{label:"",price:""}]}))} style={{...BTNS,background:"#8E44AD",marginTop:4}}>+ 옵션 추가</button>
        </div>
