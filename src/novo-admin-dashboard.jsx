                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 8 }}>
                    {(pf.tags || []).map((tag, i) => (
                      <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 4, background: tag.color || "#EAF5FA", color: tag.textColor || "#2980B9", padding: "4px 10px", borderRadius: 20, fontSize: 12, fontWeight: 600 }}>
                        {tag.label}
                        <button onClick={() => { const nt = [...(pf.tags || [])]; nt.splice(i, 1); setPf((prev) => ({ ...prev, tags: nt })); }} style={{ background: "none", border: "none", cursor: "pointer", color: "inherit", fontSize: 14, padding: 0, marginLeft: 2 }}>✕</button>
                      </span>
                    ))}
                  </div>
                    ...
                    {[
                      ...
                      <button
                        ...
                        onClick={() => {
                          ...
                          setPf((prev) => ({ ...prev, tags: [...(prev.tags || []), preset] }));
                        }}
                        ...
                        <div key={i} style={{ position: "relative", borderRadius: 8, overflow: "hidden", border: `1px solid ${c.border}`, background: "#F0EDE8" }}>
                          {m.type === "video" ? (
                            ...
                            <div style={{ width: "100%", height: 90, display: "flex", alignItems: "center", justifyContent: "center" }}>
                              <input value={m.url} onChange={(e) => { const nm = [...(pf.media || [])]; nm[i] = { ...nm[i], url: e.target.value }; setPf((prev) => ({ ...prev, media: nm })); }} placeholder="URL" style={{ width: "80%", border: `1px solid ${c.border}`, borderRadius: 4, padding: "4px 6px", fontSize: 10, textAlign: "center" }} />
                            </div>
                          )}
                          <button onClick={() => { const nm = [...(pf.media || [])]; nm.splice(i, 1); setPf((prev) => ({ ...prev, media: nm })); setPendingFiles((pf2) => pf2.filter((f) => f.index !== i).map((f) => f.index > i ? { ...f, index: f.index - 1 } : f)); }} style={{ position: "absolute", top: 3, right: 3, width: 20, height: 20, borderRadius: "50%", border: "none", background: "rgba(0,0,0,0.6)", color: "#FFF", fontSize: 11, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", padding: 0 }}>✕</button>
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  {(pf.tiered || []).map((tier, i) => (
                    <div key={i} style={{ display: "flex", gap: 6, marginBottom: 6, alignItems: "center" }}>
                      <div style={{ flex: 1 }}>
                        <input value={tier.qty} onChange={(e) => { const nt = [...(pf.tiered || [])]; nt[i] = { ...nt[i], qty: Number(e.target.value) }; setPf((prev) => ({ ...prev, tiered: nt })); }} type="number" placeholder="수량" style={{ ...inp, padding: "6px 10px", fontSize: 12 }} />
                      </div>
                      <span style={{ fontSize: 12, color: c.tLight }}>개 이상 →</span>
                      <div style={{ flex: 1 }}>
                        <input value={tier.price} onChange={(e) => { const nt = [...(pf.tiered || [])]; nt[i] = { ...nt[i], price: Number(e.target.value) }; setPf((prev) => ({ ...prev, tiered: nt })); }} type="number" step="0.01" placeholder="개당 가격" style={{ ...inp, padding: "6px 10px", fontSize: 12 }} />
                      </div>
                      <span style={{ fontSize: 12, color: c.tLight }}>$</span>
                      <button onClick={() => { const nt = [...(pf.tiered || [])]; nt.splice(i, 1); setPf((prev) => ({ ...prev, tiered: nt })); }} style={{ ...btnS, padding: "4px 8px", background: c.danger }}>✕</button>
                    </div>
                  ))}
