...
export default function Admin() {
  // Simple password gate (client-side only — use Firebase Auth for production)
  const [authed,setAuthed]=useState(()=>sessionStorage.getItem("novo_admin_ok")==="1");
  const [pwInput,setPwInput]=useState("");
  const [pwErr,setPwErr]=useState("");
  const ADMIN_PW="5711"; // TODO: replace with Firebase Auth
  const handleLogin=()=>{if(pwInput===ADMIN_PW){setAuthed(true);sessionStorage.setItem("novo_admin_ok","1");setPwErr("");}else{setPwErr("비밀번호가 틀렸습니다");}};
  ...
  // Password gate — block all rendering until authenticated
  if(!authed){
    return (
      <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,minHeight:"100vh",display:"flex",alignItems:"center",justifyContent:"center",padding:20}}>
        ...
        <div style={{background:"#FFF",borderRadius:16,padding:32,maxWidth:360,width:"100%",boxShadow:"0 4px 20px rgba(0,0,0,.08)",border:`1px solid ${C.border}`}}>
          ...
          <input type="password" value={pwInput} onChange={e=>{setPwInput(e.target.value);setPwErr("");}} onKeyDown={e=>{if(e.key==="Enter")handleLogin();}} autoFocus style={INP}/>
          {pwErr&&<div style={{background:C.dLight,color:C.danger,padding:"8px 12px",borderRadius:8,fontSize:12,marginTop:8}}>{pwErr}</div>}
          <button onClick={handleLogin} style={{...BTN,width:"100%",marginTop:16}}>로그인</button>
        </div>
      </div>
    );
  }
  ...
  return (
    <div style={{fontFamily:"'DM Sans',sans-serif",background:C.bg,minHeight:"100vh"}}>
      ...
      <div style={{padding:20,maxWidth:1100,margin:"0 auto"}}>
        ...
        {loaded&&tab==="inventory"&&<div><h2 style={{fontFamily:"'DM Serif Display',serif",fontSize:24,marginBottom:16}}>📋 재고 관리</h2>{products.filter(p=>p.stock<=5&&p.active).length>0&&<div style={{background:C.wLight,border:`1px solid ${C.warn}33`,borderRadius:12,padding:"14px 16px",marginBottom:16}}><div style={{fontWeight:700,fontSize:13,color:C.warn,marginBottom:6}}>⚠️ 재고 부족 / 품절</div>{products.filter(p=>p.stock<=5&&p.active).map(p=><div key={p.id} style={{fontSize:12,padding:"2px 0"}}>{p.image||"📦"} {p.nameKo} — <strong style={{color:p.stock===0?C.danger:C.warn}}>{p.stock===0?"품절":`${p.stock}개`}</strong></div>)}</div>}<div style={{background:"#FFF",borderRadius:14,border:`1px solid ${C.border}`,overflow:"auto"}}><table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}><thead><tr style={{background:"#F8F9FA"}}>{["제품","카테고리","가격","재고","수정"].map(h=><th key={h} style={{padding:"12px 16px",textAlign:h==="제품"?"left":"center",fontWeight:700,color:C.tLight,fontSize:11,textTransform:"uppercase"}}>{h}</th>)}</tr></thead><tbody>{products.filter(p=>p.active).sort((a,b)=>(a.nameKo||"").localeCompare(b.nameKo||"")).map(p=><tr key={p.id} style={{borderTop:`1px solid ${C.border}`}}><td style={{padding:"10px 16px"}}><div style={{display:"flex",alignItems:"center",gap:8}}>{p.media?.length>0&&p.media[0].url?<div style={{width:28,height:28,borderRadius:4,overflow:"hidden",flexShrink:0}}><img src={p.media[0].url} alt="" style={{width:"100%",height:"100%",objectFit:"contain"}} loading="lazy"/></div>:<span style={{fontSize:20}}>{p.image||"📦"}</span>}<span style={{fontWeight:600}}>{p.nameKo}</span></div></td><td style={{padding:"10px 16px",textAlign:"center",color:C.tLight}}>{categories[p.category]||p.category}</td><td style={{padding:"10px 16px",textAlign:"center",fontWeight:700}}>${(p.sale?p.salePrice:p.price||0).toFixed(2)}</td><td style={{padding:"10px 16px",textAlign:"center"}}><span style={{fontWeight:700,padding:"3px 10px",borderRadius:6,fontSize:12,background:p.stock===0?C.dLight:p.stock<=5?C.wLight:C.pLight,color:p.stock===0?C.danger:p.stock<=5?C.warn:C.primary}}>{p.stock}</span></td><td style={{padding:"10px 16px",textAlign:"center"}}><div style={{display:"flex",gap:4,justifyContent:"center",alignItems:"center"}}><button onClick={()=>handleStockChange(p.id,p.stock-1)} style={{...BTNS,padding:"4px 8px",background:C.tLight}}>−</button><input value={p.stock} onChange={e=>handleStockChange(p.id,parseInt(e.target.value)||0)} style={{width:50,textAlign:"center",border:`1px solid ${C.border}`,borderRadius:6,padding:4,fontSize:13,fontFamily:"'DM Sans',sans-serif"}}/><button onClick={()=>handleStockChange(p.id,p.stock+1)} style={{...BTNS,padding:"4px 8px"}}>+</button></div></td></tr>)}</tbody></table></div></div>}
      </div>
    </div>
  );
}
